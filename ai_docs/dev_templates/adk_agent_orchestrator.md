...
- **User-Controlled**: User decides when to invoke the agent
- **Tool-like Behavior**: Agent provides a specific service/capability
...
- **Automated Flow**: Agent executes automatically without user intervention
...
1. **Does the user need control over when this agent executes?** → Use AgentTool
2. **Is this agent always part of the workflow?** → Use Sub-Agent
3. **Might this agent be called multiple times?** → Use AgentTool
4. **Is this a conditional capability?** → Use AgentTool
5. **Is this a required pipeline step?** → Use Sub-Agent
...
interactive_planner_agent = LlmAgent(
    ...
    model="gemini-2.5-flash",
    tools=[
        AgentTool(plan_generator),  # Plan generation as tool
    ],
    sub_agents=[
