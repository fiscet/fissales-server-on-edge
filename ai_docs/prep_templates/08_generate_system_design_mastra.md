...
If the template is mastra-agent-saas or similar Mastra.AI template, MANDATORY requirements:
- **Individual Component Breakdown** - Show each Agent, Workflow, Tool separately
- **Mastra Server Separation** - Create "Mastra.AI Server" subgraph separate from Next.js app
- **API Connection Visualization** - Use dotted lines to show API calls between web app and Mastra server
- **Component Type Labels** - Clearly label component types: (Agent), (Workflow), (Tool)
- **State Management** - Show Memory storage and Runtime Context infrastructure
- **Data Flow Pattern** - Show how Mastra components communicate with web app APIs
...
- **Mastra Component Breakdown** (for mastra-agent templates): Show individual agents, workflows, and tools with API connections to Next.js app
...
subgraph "Mastra.AI Server - [Custom Agent System]"
    subgraph "Root Agent"
        [Main orchestrating agent (Mastra Agent)]
    end
    subgraph "Sequential/Parallel Workflows"
        [Workflows with .then() or .parallel() steps]
    end
    subgraph "Individual Agents"
        [Specific Agents for each workflow step]
    end
    subgraph "Quality Assurance" (optional)
        [Loop workflows with .dowhile()/.dountil(), validation agents]
    end
    subgraph "Mastra Infrastructure"
        [Memory storage, Runtime Context, MCP Server]
    end
end
...
- **Mastra Agent Server Layer** (for mastra-agent templates) - MANDATORY separate layer showing individual agents and workflows
- **API Connection Visualization** - Use dotted lines (-.->|label|) to show API calls between Next.js and Mastra server
- **Component Type Specification** - Label each component with its Mastra type: (Agent), (Workflow), (Tool), etc.
...
- Individual Agents: "Idea Clarification Agent (Agent)", "Hook Creation Agent (Agent)"
- Workflows: "Sequential Workflow (.then())", "Quality Loop (.dountil())"
- API Connections: "AgentAPI -.->|HTTP API calls| RootAgent", "RootAgent -.->|Callback API| CallbackAPI"
- Mastra Infrastructure: "Memory Storage", "Runtime Context", "MCP Server"
...
- Missing Component Breakdown: Not showing individual Agents, Workflows, Tools
- No API Boundaries: Not showing separation between Next.js app and Mastra server
- Oversimplified: "YouTube Agent" without showing internal agent/workflow structure]
```
...
- **Mastra-Agent-SaaS + custom workflow** → Design custom agent system with individual Agents and Workflows
...
- **Break down component hierarchy** - Root agent → Workflow orchestration → Individual agent steps → Tool integrations
- **Show API boundaries** - Dotted lines between web app and Mastra server
- **Include Mastra infrastructure** - Memory Storage, Runtime Context, MCP Server
- **Demonstrate state flow pattern** - How agents/workflows communicate via state and runtime context
...
- **Mastra-specific language** (for mastra-agent templates) - "Your Mastra server will have X agents and Y workflows, with components communicating via API"
...
- **MANDATORY Mastra breakdown** (for mastra-agent templates) - Always show individual agents, workflows, tools and API connections
...
- **Perfect Mastra Depth:** "Your Mastra server needs a Sequential Workflow with 6 Agent steps (Idea, Hook, Title, Thumbnail, Email, Social) communicating via workflow state"
- **Too Deep:** "Create uploadHandler() function that calls Cloud Storage API"
- **Too Mastra Deep:** "Create IdeaAgent class with specific Zod schemas and tool configuration details"
